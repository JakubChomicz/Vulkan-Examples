...
namespace Example
{
	...
	{
		return HML::Vector3<>(0.0f, 0.0f, -1.0f);
	}
	...
	void Camera::UpdateView()
	{
		...
		const auto sineAzimuth = sin(m_Yaw);
		const auto cosineAzimuth = cos(m_Yaw);
		const auto sinePolar = sin(m_Pitch);
		const auto cosinePolar = cos(m_Pitch);

		// Calculate eye position out of them
		const auto x = HML::Vector3<>(0.0f, 0.0f, 0.0f).x + m_Position.z * cosinePolar * cosineAzimuth;
		const auto y = HML::Vector3<>(0.0f, 0.0f, 0.0f).y + m_Position.z * sinePolar;
		const auto z = HML::Vector3<>(0.0f, 0.0f, 0.0f).z + m_Position.z * cosinePolar * sineAzimuth;
		if (m_Position.z < 2.0f)
			m_Position.z = 2.0f;

		HML::Vector3<> eye = HML::Vector3<>(x, y, z);

		m_data.View = HML::Transform::View_RH(eye, HML::Vector3<>(0.0f, 0.0f, 0.0f), -HML::Vector3<>(0.0f, -1.0f, 0.0f));
	}
